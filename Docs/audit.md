...
- **Verification Date**: 2026-08-11.
  ...
  - **Deployed & Verified Bundle**: Edge function `process-voice-job` deployed live to Supabase project `lyowklxsbfznnqridtgr` on 2026-08-11. Downloaded live bundle via `supabase functions download` and grepped to confirm all 4 markers (`ISSUE-126`, `resolveConceptToSkus`, `ambiguousConceptReason`, `conceptOfSpoken(rawName) ?? conceptOfSku(rawName)`) are present in the deployed code.
  - **Pending live test phone recordings**: End-to-end trace verification awaits recording the 2 test utterances ("सत्रह किलो चावल" and "पांच किलो दूध") on the physical test phone.
...
- **Status**: SCHEMA + BACKFILL LIVE, STAGE 1 + 1b CODE COMPLETE + UNIT-TESTED, **EDGE FUNCTION DEPLOYED 2026-08-11 (BUNDLE MARKERS VERIFIED)**, pending live test phone recordings for end-to-end trace verification. Kotlin client mirror (`ItemLexicon.kt`, `OrderingSegmenter`) is Stage 2 and untouched — the offline path keeps its current surface-list behaviour.
