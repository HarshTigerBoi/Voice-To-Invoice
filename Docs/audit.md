...
| **STT / Chat Model Ids** | `XAI_CHAT_MODEL` (default `grok-2-latest`), `SARVAM_STT_MODEL` (default `saarika:v2`) | `process-voice-job/index.ts` (top of file) | Env-configurable as of ISSUE-020; defaults preserve prior behaviour. `grok-2-latest` is an old chat model and a known weak point in step 4 — bump via env var. Sarvam has been returning empty transcripts for unknown reasons; the cause now surfaces in `step_2_stt_proxy_response.sarvamStt.error`. |
