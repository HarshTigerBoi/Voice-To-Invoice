...
| **Sale Plausibility** | GRAM/ML `<10` or `>5000`; KG/LITRE `>200`; PIECE/PACKET/DOZEN `>500`; quantity `<=0`; total `< ₹5` (`MIN_PLAUSIBLE_SALE_VALUE`) | `SalePlausibility.kt` & `implausibilityReason()` in `process-voice-job/index.ts` (mirrored) | Added in ISSUE-022 after "7 GRAM Jeera / ₹2.80" auto-confirmed. **Never blocks a sale** — only withholds auto-confirm and routes to review with a reason string in the trace. Wired into both the server `isAutoConfirmed` and client `isAutoConfirmable` gates. |
...
- **Verification Date**: 2026-07-26. Code change only — **not yet deployed** to the live edge function (`lyowklxsbfznnqridtgr`) and not exercised against a real job; verify by redeploying `process-voice-job` and re-running a bare "`<item>` `<price>` रुपये" utterance, then confirming (a) no new row in `transactions` for that job_id and (b) `catalog_items.price` updated for the matched item.
