...
- **Related defect surfaced, NOT yet fixed**: `db-setup/index.ts:141-144` seeds `दूध`/`doodh`/`milk`/`dudh` → **`Amul Gold Milk`**, silently choosing one of three milk SKUs spanning ₹27–₹34. Same class as ISSUE-109. Under this design those must resolve to concept `milk` and ask. The ambiguity path is wired in `concepts.ts` but **not yet called from `index.ts`** — deferred with the rest of Stage 1b.
- **Verification Date**: 2026-08-11.
  - **Verified**: `node --experimental-strip-types --test` over `concepts_test.ts`, `item_resolution_test.ts`, `price_intent_test.ts`, `multi_item_test.ts`, `lexicon_test.ts`, `phonetic_test.ts`, `unit_conversion_test.ts` → **80/80 pass**. `concepts_test.ts` asserts against the real 53-SKU live catalog read from Supabase, not invented names.
...
- **Status**: SCHEMA + BACKFILL LIVE, CODE COMPLETE + UNIT-TESTED, **NOT DEPLOYED, NEVER EXERCISED**. Kotlin client mirror (`ItemLexicon.kt`, `OrderingSegmenter`) is Stage 2 and untouched — the offline path keeps its current surface-list behaviour.
