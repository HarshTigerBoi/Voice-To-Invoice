...
- **Symptom**: Two jobs 13 seconds apart, same commodity, opposite results.
  ...
  - Job `735469d9` "सत्रह किलो चावल" → segmenter matched `चावल` at **0.0** (a surface of `ITEM_LEXICON` canonical `Chawal`, lexicon.ts:133), below the threshold, so the override replaced the AI's "Basmati Rice" with "चावल" — a vocabulary word with no `catalog_items` row. `matchedCatalogId: null`, ₹0, confidence 0.55, routed to `unmatched_queue`.
  - `step_4_raw_ai_items` was `"Basmati Rice"` in **both** jobs. The AI was right twice; the deterministic override discarded it once.
- **Root Cause**:
  1. The system had no representation of *what an item is*. Four name-spaces (`DEFAULT_ITEM_VOCAB`, `catalog_items`, `term_aliases`, `ITEM_LEXICON`) are all **surface-form** lists, so cross-language identity had to be hand-enumerated and any missed spelling failed silently.
  2. Phonetics cannot bridge translations by construction: `phoneticKey('rice')` = `LICI`, `phoneticKey('चावल')` = `CAVAL`. Zero shared phones. Adding more phonetic matching makes this *worse* — it is what produced "Rice" → "Elaichi".
  3. `resolveItemName` arbitrated on phonetic string distance alone, so a translation pair was indistinguishable from a mishearing.
- **Resolution** (Stage 1, server + schema):
  1. New `supabase/functions/process-voice-job/concepts.ts` — `CONCEPTS` (base-commodity ids), `conceptOfSpoken`, `conceptOfSku`, `resolveConceptToSkus`, `ambiguousConceptReason`. Division of labour is explicit: phonetics for sound-alike spellings (chawal/chaval), concept ids for translations, the LLM for anything in neither.
  2. Migration `20260811000000_add_catalog_concept.sql` — nullable `catalog_items.concept` + `(shop_id, concept)` index. A NULL row behaves exactly as before, so it cannot regress an unbackfilled shop.
  3. `resolveItemName` gained an optional `ConceptArbitration` argument: when both readings share a concept **and** the AI's name is a stocked SKU, the sellable name wins. Gated on both conditions so **ISSUE-030 is untouched** — `अमचूर`/`अंगूर` are concepts `amchur`/`grapes`, still disagree, segmenter still wins, including when the AI's wrong word is stocked. Omitting the argument is byte-identical to prior behaviour (regression-tested).
  4. Narrowing within an ambiguous concept uses **phonetic token overlap against the candidate SKU names**, deliberately NOT the `QUALIFIERS` list — registering every product line (Gold, Taaza, Bourbon, Parle-G, Chana, Moong, Mustard) is the same unbounded enumeration this layer exists to remove.
- **Rejected alternatives, with reasons**: vector embeddings would rank Amul Gold (₹34) / Amul Taaza (₹27) / Saras (₹30) within noise of each other — near-identical names, real money apart — and break offline-first parsing on the client. Metaphone-family phonetics is already implemented in Devanagari-aware form and cannot link translations. Algolia/Elasticsearch synonyms means search infrastructure for a 53-row table. See `Docs/concept_layer_plan.md`.
- **Live catalog finding**: 41 of the pilot shop's 53 SKUs are unambiguous (rice, sugar, ghee, atta, salt, tea… resolve cleanly). Exactly 4 concepts are ambiguous: **milk** (3), **biscuit** (4), **dal** (3), **oil** (2).
- **Related defect surfaced, NOW FIXED (Stage 1b, same day)**: `db-setup/index.ts:141-144` seeds `दूध`/`doodh`/`milk`/`dudh` → **`Amul Gold Milk`**, silently choosing one of three milk SKUs spanning ₹27–₹34 via the exact-distance alias short-circuit in `matchVocab` (`phonetic.ts`) — the segmenter's `itemMatchNorm: 0` for "दूध" was never a real catalog match, it was this alias firing. `index.ts` now computes `finalNameConcept = conceptOfSpoken(rawName) ?? conceptOfSku(rawName)` and calls `resolveConceptToSkus` immediately after the `noPriceForUnit` check (same cascade, same style); an `AMBIGUOUS` result appends `ambiguousConceptReason(...)` to `implausibility_reason`, which caps confidence at `IMPLAUSIBLE_CONFIDENCE_CAP` (0.55) via the pre-existing gate at the bottom of the per-item loop — the same mechanism `noPriceForUnit`/`isBasketTotalAmbiguous`/every other safety check already uses, so no new gating logic was introduced. Deliberately independent of `matched`/`isCatalogMatched`: it catches the case where the alias mechanism *already* produced a confident but wrong pick, and it also stops `resolveItemName`'s new concept-arbitration (this same issue, above) from trusting an AI hallucination just because it happens to share a concept and be stocked — for an ambiguous concept, this check fires regardless of which of the two guesses won upstream, so trace `b6ebbef5` (दूध → Amul Gold Milk vs Saras Milk) is blocked either way. 3 new tests in `concepts_test.ts` assert the ambiguity call with the exact shapes `index.ts` builds (both `'Amul Gold Milk'` and `'Saras Milk'` as `rawName` independently classify AMBIGUOUS against the live catalog; `चावल`/`Basmati Rice` and unambiguous concepts like sugar are confirmed NOT blocked). 83/83 tests pass.
- **Verification Date**: 2026-08-11.
  - **Verified**: `node --experimental-strip-types --test` over `concepts_test.ts`, `item_resolution_test.ts`, `price_intent_test.ts`, `multi_item_test.ts`, `lexicon_test.ts`, `phonetic_test.ts`, `unit_conversion_test.ts` → **83/83 pass**. `concepts_test.ts` asserts against the real 53-SKU live catalog read from Supabase, not invented names. `node --experimental-strip-types --check index.ts` confirms the Stage 1b wiring is syntactically valid (cannot be executed directly in Node — it imports the Deno `serve` runtime — so this is a syntax check, not a run).
  - **Verified live**: migration applied to `lyowklxsbfznnqridtgr`; all 53 rows backfilled; re-queried post-backfill confirming zero NULL concepts and exactly the 4 ambiguous groups above.
  - **NOT verified — the edge function is NOT DEPLOYED.** `npx supabase functions deploy` failed with `LegacyPlatformAuthRequiredError` (no `SUPABASE_ACCESS_TOKEN` in this cloud session), and the MCP deploy path requires inlining ~250KB of source, which risks a truncated bundle — the exact silent-partial-deploy failure this repo has a history of. **No live voice job has exercised any of this.** The schema change and backfill ARE live; the code that reads them is not. Until deployed, behaviour on the phone is unchanged.
  - **Known gap, recorded not hidden**: Devanagari `ज़` folds to `j` (class C) while Latin `z` maps to class S, so "ताज़ा" vs "Taaza" sits at 0.25 and does not narrow. Same pre-existing gap affects `प्याज`/`pyaz`, masked only because `ITEM_LEXICON` lists both surfaces. Fails **safe** — the shopkeeper is asked which milk rather than charged for the wrong one. Fixing it means merging `z` into class C in `phoneticKey`, a core-engine change mirrored into Kotlin; out of Stage 1 scope.
- **Status**: SCHEMA + BACKFILL LIVE, STAGE 1 + 1b CODE COMPLETE + UNIT-TESTED, **EDGE FUNCTION NOT DEPLOYED, NEVER EXERCISED ON A LIVE RECORDING**. Kotlin client mirror (`ItemLexicon.kt`, `OrderingSegmenter`) is Stage 2 and untouched — the offline path keeps its current surface-list behaviour.
