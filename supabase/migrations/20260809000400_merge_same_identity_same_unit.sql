...
BEGIN
    FOR grp IN
        SELECT shop_id, canonical_key, base_unit, array_agg(id) AS ids, count(*) AS cnt
        FROM catalog_items
        WHERE active
        GROUP BY shop_id, canonical_key, base_unit
        HAVING count(*) > 1
    LOOP
        ...
        IF conflict_found THEN
            ...
            RAISE NOTICE 'CANONICAL_MERGE_CONFLICT shop=% canonical=% base_unit=% ids=% prices=% units=%',
                grp.shop_id, grp.canonical_key, grp.base_unit, loser_ids, prices, units;
        ELSE
